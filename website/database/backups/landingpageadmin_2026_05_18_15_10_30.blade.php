    ...
    <style>
        ...
        @media (min-width: 1024px) {
            ...
            .hero-gradient {
                ...
                inset: 0;
                z-index: 10;
                background: linear-gradient(to right, #f9f9fe 0%, #f9f9fe 0%, rgba(249, 249, 254, 0) 100%) !important;
            }
        }
        .hero-gradient {
            display: none;
        ...
        #loading-screen {
            ...
            z-index: 10000;
            overflow: hidden;
            background-color: #1D5083 !important; /* Solid background at start */
        }
        .panel {
            position: absolute;
            inset: 0;
...
<body class="landingpage1 min-h-screen relative overflow-x-hidden " style="background-color: rgba(249, 249, 254, 0.1);">
    <!-- Cinematic Loading Screen -->
    <div id="loading-screen">
        <!-- SVG Panels and Crack -->
        <svg class="absolute inset-0 w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
            <!-- Left Panel -->
            <g id="panel-left-group">
                <path d="M -1 0 L 50 0 L 49 15 L 52 30 L 47 50 L 53 70 L 48 85 L 50 100 L -1 100 Z" fill="#1D5083" stroke="#1D5083" stroke-width="1" />
                <!-- Crack Path Left (Moves with left panel) -->
                <path class="crack-line" d="M 50 0 L 49 15 L 52 30 L 47 50 L 53 70 L 48 85 L 50 100" stroke="rgba(255, 255, 255, 0.9)" stroke-width="3" fill="none" style="opacity: 0;" />
            </g>
            
            <!-- Right Panel -->
            <g id="panel-right-group">
                <path d="M 101 0 L 50 0 L 49 15 L 52 30 L 47 50 L 53 70 L 48 85 L 50 100 L 101 100 Z" fill="#1D5083" stroke="#1D5083" stroke-width="1" />
                <!-- Crack Path Right (Moves with right panel) -->
                <path class="crack-line" d="M 50 0 L 49 15 L 52 30 L 47 50 L 53 70 L 48 85 L 50 100" stroke="rgba(255, 255, 255, 0.9)" stroke-width="3" fill="none" style="opacity: 0;" />
            </g>
        </svg>
    </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            // Set initial dasharray and dashoffset for the crack lines
            const crackLines = document.querySelectorAll(".crack-line");
            ...
            // Stage 1 & 2: Silent Pause (Solid blue)
            tl.to({}, {duration: 1.0})

            // Rev 2: Efek gempa bumi kecil
            .to("#loading-screen", {
                x: "+=3",
                yoyo: true,
                repeat: 4,
            ...
            // Rev 2: Terjadi retakan kecil (smooth, tidak tiba-tiba)
            .fromTo(".crack-line", {
            ...
            // Rev 3: Efek gempa bumi yang lebih kuat
            .to("#loading-screen", {
                x: "+=10",
                yoyo: true,
                repeat: 8,
            ...
            // Rev 3: Terjadi retakan besar (widening) selama gempa kuat
            .to(".crack-line", {
                strokeWidth: 5,
                duration: 0.4,
                ease: "expo.out"
            ...
            // Stage 4: Screen Splitting (IMMEDIATELY after crack)
            // We split the crack by moving the panels apart.
            // Since the crack lines are inside the groups, they will split and follow the panels!
            // We also add a slight Y movement to simulate following the jagged crack pattern.
            .to("#panel-left-group", {
                xPercent: -100,
                yPercent: -2,
                duration: 2.0,
            ...
            // Fade out the loading screen background to reveal the page
            .to("#loading-screen", {
                backgroundColor: "rgba(29, 80, 131, 0)",
                duration: 0.5,
                ease: "power2.out"
            }, "<")

            // Softly fade in the page content behind the splitting panels
            .to("#app-content", {
                opacity: 1,
                duration: 1.5,
                ease: "power2.out"
            ...
            // Remove the loading screen from view
            .to("#loading-screen", {
                display: "none",
                duration: 0.1
            });
    ...
    <!-- Header / Nav -->
        <header class="fixed top-0 left-0 right-0 z-50 bg-[#1D5083] h-20 flex items-center px-10 shadow-lg">
        <div class="max-w-[1440px] w-full mx-auto flex justify-between items-center">
            <!-- Logo -->
            <div class="flex items-center gap-2 cursor-pointer group">
                <!-- Lucide Scale Icon -->
                <!-- WARNA IKON DIGANTI JADI text-white -->
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-white w-8 h-8 group-hover:scale-105 transition-transform"><path d="m16 16 3-8 3 8c-.87.65-1.92 1-3 1s-2.13-.35-3-1Z"/><path d="m2 16 3-8 3 8c-.87.65-1.92 1-3 1s-2.13-.35-3-1Z"/><path d="M7 21h10"/><path d="M12 3v18"/><path d="M3 7h2c2 0 5-1 7-2 2 1 5 2 7 2h2"/></svg>
                <!-- WARNA TEKS DIGANTI JADI text-white -->
                <span class="font-serif text-2xl text-white italic tracking-tight">Lawsy</span>
            </div>
            <!-- Action Button -->
            <!-- WARNA TOMBOL DIGANTI JADI BG PUTIH, TEKS BIRU -->
            <a href="{{ route('signin') }}" class="bg-white text-[#1D5083] hover:bg-gray-100 font-semibold py-2.5 px-6 rounded-full flex items-center gap-2 text-sm transition-all shadow-sm">
                Access Firm Portal
    ...
    <!-- Hero Section (50/50 Split Grid) -->
    <section class="relative min-h-screen flex flex-col hero-grid bg-[#f9f9fe]">
        <!-- Left Content Area -->
        <div class="w-full hero-left flex items-center justify-center p-12 lg:p-24 z-20 relative">
            <!-- Soft Blur Circles -->
            <div class="absolute top-1/4 left-1/4 w-64 h-64 bg-blue-100 rounded-full filter blur-3xl pointer-events-none z-0 opacity-20"></div>
            <div class="absolute bottom-1/4 left-1/3 w-96 h-96 bg-gray-100 rounded-full filter blur-3xl pointer-events-none z-0 opacity-30"></div>
            
            <div class="max-w-2xl w-full relative z-10" data-aos="fade-up" data-aos-duration="800">
                
                <!-- Badge -->
                <div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-blue-200 bg-blue-50 backdrop-blur-md mb-8">
                    <!-- Lucide ShieldCheck Icon -->
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color: #003966;" class="w-4 h-4"><path d="M20 13c0 5-3.5 7.5-7.66 8.95a1 1 0 0 1-.67-.01C7.5 20.5 4 18 4 13V6a1 1 0 0 1 1-1c2 0 4.5-1.2 6.24-2.72a1.17 1.17 0 0 1 1.52 0C14.51 3.81 17 5 19 5a1 1 0 0 1 1 1z"/><path d="m9 12 2 2 4-4"/></svg>
                    <span style="color: #003966;" class="text-[10px] sm:text-xs font-bold uppercase tracking-widest">
                        Enterprise Grade Security
                    </span>
                </div>

                <!-- Headlines -->
                <h1 class="text-5xl md:text-7xl leading-[1.1] text-[#1a1c1f] mb-6 tracking-tight font-serif">
                    Elite Jurisprudence.<br />
                    <span style="color: #003966;" class="italic">Powered by Precision<br />Intelligence.</span>
                </h1>
                ...
                <div class="flex flex-wrap items-center gap-4">
                    <a href="{{ route('signin') }}" style="background-color: #1d5083;" class="hover:bg-[#003966] text-white font-semibold py-4 px-8 rounded-full shadow-lg transition-all active:scale-95 text-center flex items-center gap-2 justify-center">
                        Access Firm Portal
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-5 h-5"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>
                    </a>
                   <button style="color: #003966;" class="border border-[#003966] flex items-center gap-2 font-semibold py-4 px-6 hover:bg-[#EBF5FF] rounded-full transition-colors">
    <!-- Lucide PlayCircle Icon -->
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-6 h-6"><circle cx="12" cy="12" r="10"/><polygon points="10 8 16 12 10 16 10 8"/></svg>
    View Capabilities
</button>
        ...
        <!-- Right Image Area -->
        <div class="w-full hero-right relative z-10" style="min-height: 50vh;">
            <img src="https://images.unsplash.com/photo-1505664194779-8beaceb93744?auto=format&fit=crop&q=80&w=2000" class="absolute inset-0 w-full h-full object-cover muted-cinema z-0" />
            <!-- Smooth fade transition to the white background (Desktop) -->
            <div class="hero-gradient"></div>
            <!-- Gradient for mobile -->
            <div class="block lg:hidden absolute inset-0 bg-gradient-to-t from-[#f9f9fe] via-[#f9f9fe] to-transparent" style="opacity: 0.8;"></div>
        </div>
    ...
    <section class="relative py-20 bg-cover bg-center bg-no-repeat mt-0" style="background-image: url('/images/bookshelf_bg.png');">
        ...
        <section class="landingpage7 bg-white border-t border-[#E5E7EB]/50">
            ...
            <div class="py-24 flex flex-col items-center justify-center text-center gap-8">
                ...
                <a href="{{ route('signin') }}" class="bg-[#1D5083] text-white px-8 py-4 rounded-full text-base font-bold shadow-lg shadow-[#1D5083]/20 hover:bg-[#153c63] transition-all duration-300 transform hover:-translate-y-0.5 flex items-center gap-2 justify-center">
                    Access Firm Portal
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-5 h-5"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>
                </a>
            </div>
